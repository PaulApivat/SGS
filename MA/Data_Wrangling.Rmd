...
# Select 3-4 variables (columns) from data_clmv
# then pipe the summary() function from those variables (also try str() and view() )
data_clmv %>%
    select(GeoAreaName, Value, Units, Sex, `Type of skill`, TimePeriod, Time_Detail) %>%
    summary()
...
# Use the mutate() function to change the variable type within a piping operation in data_clmv
...
data_clmv %>%
    mutate(
        ...
        indicator_dbl = as.factor(Indicator),
...
# Aggregate across Gender (Male, Female, Bothsex), we want to look at the aggregate proportion ranked by ICT Skills, for Cambodia in 2017?
...
data_clmv %>%
    ...
    # group_by
    group_by(type_of_skill) %>%
    # When aggregating across Gender within a Year, we can use the mean_value 
    summarize(
        mean_value = mean(Value)
    ) %>%
    ungroup() %>%
...
## YOUR TURN: Aggregating Across TimePeriod
...
## YOUR TURN: Asking Specific Questions of the Data
