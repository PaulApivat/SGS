...
I kept the above for you to see what is provided everytime you click on File -> New File -> RMarkdown. The primary thing for you to know about RMarkdown (for now) is that its a format that you can use to run your analysis and, most importantly, communicate your findings to your stakeholders. 

Below we will do some fundamental things that you'll do at the start of every data handling session. 
...
Here we'll read in the data that is in Excel format. I've stored raw data in the 'data' folder for this project.
...
# use melt() function from reshape2 pakcage to turn from wide into long format
# now melt_multiple_histo has two columns
melt_multiple_histo <- melt(multiple_histo)

# plot with facetted boxplots to see four boxplots at the same time
